...
- **フィルター・並び替えパネル**: リスト上部の常駐パネルで絞り込みと並び替えを操作します。編集モード中はパネル全体が半透明になり操作不可になりますが、レイアウトの高さは変わりません。
    - **上段（カテゴリ・並べ替え）**: カテゴリ絞り込みと並び替えの2つのセレクトボックスが横並びで配置されています。
        ...
        - 並び替えは「在庫が少ない順（デフォルト）」「在庫が多い順」「更新が古い順」「更新が新しい順」の4択です。「更新が古い順」「更新が新しい順」を選択中は、各カードに最終更新日（`更新 M/D` 形式、前年以前は `YYYY/M/D`）が表示されます。更新日ソート時はアーカイブ済み商品も日付順で並びます。
    - **下段（在庫ステータスフィルタ）**: ✓（十分）・🛒（要購入）・−（未判断）の3つのアイコンボタンが横幅いっぱいに並びます。
        - 複数選択可能なトグル式で、選択中のボタンはそのステータスのカラーで強調表示されます。
        - 何も選択していない場合はすべて表示。選択している場合は該当ステータスの商品のみ表示されます。
        - ステータスフィルタが有効なとき、継続購入OFF（アーカイブ済み）の商品は除外されます。
