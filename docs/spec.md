...
### 3.9 アプリ設定 (`/settings/app-settings`)
- **ブラウザローカル保存**: ユーザー固有の設定をブラウザの `localStorage` に保存し、端末ごとにカスタマイズ可能としている。
- **棚卸し基準日数**: 棚卸し機能で「未更新」と判定する日数をスライダーおよび数値入力で設定できる（1〜365日、デフォルト30日）。
