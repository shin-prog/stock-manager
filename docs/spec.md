...
### 3.1 在庫管理 (Dashboard)
- **一覧表示**: 登録されている全商品の現在の在庫数（個）を表示します。
- **クイック調整**: 「+1」「-1」ボタンにより、買い物を通さずとも手軽に在庫数を修正可能です。
- **カテゴリフィルタ**: 「キッチン」「掃除」などのカテゴリで表示商品を絞り込めます。
- **並び替え機能**: 「在庫が多い順」「在庫が少ない順」でリストをソート可能です。
...
- **商品メモ**: 各商品ごとに、特徴や使い方などを自由に保存できるメモ欄があります。
- **商品詳細（価格履歴）**:
    - 過去の購入記録を時系列で表示。
    - どのお店で、いつ、どのようなサイズ（容量・メモ）をいくらで買ったかを確認可能です。
    - **履歴の削除**: 誤った購入記録を削除でき、削除時は在庫数も自動で差し引かれます。
    - **クイック購入**: 詳細ページから、その商品を即座に購入登録できる専用フォームがあります。
